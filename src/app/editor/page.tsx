...
import {PlayIcon, BotOffIcon, RefreshCwIcon, StopCircleIcon } from "lucide-react";
  ...
  return (
    <Navbar className="flex py-32 p-8 flex-wrap justify-center gap-2 bg-g">
      <div className="h-full w-3/5">
          <Editor ref={editorRef} setActiveTab={setActiveTab} output={output} setOutput={setOutput} isRunning={isRunning} setIsRunning={setIsRunning} />
      </div>
        ...
        <Card className="h-full relative">
              ...
              <div className="flex justify-center">
                <div className="p-3 flex flex-col h-[60vh] overflow-hidden overflow-y-auto">
                  {snippets.map((snippet, index) => (
                    <CodeEditorSnippet
                      code={snippet.code}
                      handleSnippetDragStart={handleSnippetDragStart}
                      key={index}
                    />
                  ))}
                </div>
            ...
            <TabsContent value="output" className="max-h-[90%]">
              <div className="p-4 flex-grow">
                <ScrollArea className="h-full font-mono">
                  <span className="whitespace-pre-wrap">{output.join("\n")}</span>
                </ScrollArea>
              </div>
            </TabsContent>
          </Tabs>
          ...
          <div className="absolute bottom-4 right-4 left-4 flex justify-center gap-2">
            ...
            <Button onClick={handleRunCode}>
              {isRunning ? <StopCircleIcon/> : <PlayIcon className="h-6 w-6" />}
            </Button>
